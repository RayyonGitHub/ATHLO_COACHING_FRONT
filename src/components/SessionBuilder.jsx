...
const SessionBuilder = () => {
  ...
  // --- CHARGEMENT DES DONNÉES (Exercices + Programmes) ---
  useEffect(() => {
  ...
  const handleDrop = (e) => {
    ...
    if (exo) {
      setSessionExos([...sessionExos, {
        uid: crypto.randomUUID(), // API Native robuste pour l'ID unique
        exercice: exo,
        series: 3,
        repetitions: "10",
        poids: "",
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto h-[calc(100vh-5rem)] flex flex-col gap-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-end shrink-0">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Créateur de Séance</h1>
          <p className="text-slate-500 text-sm">Glissez-déposez vos exercices pour construire le programme.</p>
        </div>
        <button 
          onClick={handleSaveSession}
          disabled={isSaving || sessionExos.length === 0 || !selectedProgrammeId}
          className="bg-[#FF6A00] hover:bg-orange-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-orange-500/20 transition-all flex items-center gap-2 disabled:opacity-50"
        >
          <Save size={18} />
        ...
        <div className="w-full lg:w-2/3 flex flex-col bg-white dark:bg-[#16161A] border border-slate-200 dark:border-[#26262B] rounded-2xl overflow-hidden shadow-sm">
          <div className="p-6 border-b border-slate-200 dark:border-[#26262B] bg-slate-50/50 dark:bg-[#0B0B0F]/30 space-y-4">
            ...
            <input 
              ...
              className="w-full bg-transparent text-2xl font-black text-slate-900 dark:text-white placeholder-slate-400 outline-none"
            />
          </div>

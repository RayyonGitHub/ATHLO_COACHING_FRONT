...
const ProspectCheckout = () => {
      ...
      <div className="min-h-screen bg-[#0e0e12] text-white flex items-center justify-center p-6">
        <div className="bg-[#19191e] border border-white/10 rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-[#ff6a00] hover:bg-orange-600 font-bold"
          >
            Retour au dashboard
          </button>
        </div>
  ...
  const handlePay = async (e) => {
    ...
    try {
      const response = await prospectService.payCheckout({
        coach_id: coach.id,
      ...
      if (response.payment_status === 'success') {
        navigate(`/prospect/payment/success?token=${encodeURIComponent(response.checkout_token)}`);
      } else {
        navigate(`/prospect/payment/failed?token=${encodeURIComponent(response.checkout_token)}`);
      }
    } catch (err) {
      console.error(err);
      setError(err.response?.data?.message || 'Impossible de traiter le paiement.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="bg-[#0e0e12] fixed top-0 w-full z-50 shadow-[0_40px_60px_-15px_rgba(0,0,0,0.3)]">
        <div className="flex justify-between items-center px-6 py-4 w-full">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/prospect/dashboard')} className="text-gray-400 hover:text-white">
              <span className="material-symbols-outlined">arrow_back</span>
            </button>
            <div className="text-2xl font-black italic tracking-tighter text-[#FF6A00]">ATHLO</div>
          </div>
          <div className="hidden md:flex items-center gap-8">
            <button onClick={() => navigate('/prospect/dashboard')} className="text-[#acaab0] hover:bg-[#1f1f25] transition-colors duration-200 px-3 py-1 rounded-lg">
              Dashboard
            </button>
          </div>
        </div>
            ...
            <form onSubmit={handlePay} className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-[#131317] border-none rounded-xl px-4 py-3 outline-none text-white"
                      placeholder="email@athlo.com"
                      type="email"
                      required
                    />
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[#131317] p-6 rounded-2xl space-y-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-gray-400 ml-1">Carte</label>
                    <input
                      name="card_number"
                      value={formData.card_number}
                      onChange={handleChange}
                      className="w-full bg-[#1f1f25] border-none rounded-xl px-4 py-3 outline-none text-white"
                      placeholder="4242 4242 4242 4242"
                      type="text"
                      required
                    />
                    <p className="text-xs text-gray-500">
                      Astuce test : une carte finissant par <span className="text-red-400 font-bold">0002</span> simule un refus.
                    </p>
                  </div>
              ...
              <p className="text-center text-xs text-gray-400">
                Transaction simulée côté backend. Le flow est prêt pour être remplacé plus tard par Stripe.
              </p>
            </form>
          </div>

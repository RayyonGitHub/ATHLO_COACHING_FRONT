...
const InviteCheckout = () => {
  ...
  const total = useMemo(() => {
    return Number(preview?.offer?.price || 0);
  }, [preview]);
    ...
    const fetchPreview = async () => {
      ...
      try {
        ...
        setFormData((prev) => ({
          ...
          email: data?.email || '',
          phone: data?.phone || '',
          cardholder_name: data?.full_name || '',
        }));
      } catch (err) {
        console.error(err);
        setError(
          err.response?.data?.message ||
            "Impossible de charger l'invitation de paiement."
        );
      } finally {
        setLoadingPreview(false);
      }
  ...
  const handlePay = async (e) => {
    ...
    try {
      const response = await prospectService.payInvitationCheckout({
      ...
      if (response.payment_status === 'success') {
        navigate(`/invite/set-password?token=${encodeURIComponent(response.activation_token || token)}`);
      } else {
        setError(response.message || 'Le paiement a été refusé.');
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-[#0e0e12] text-[#fcf8fe] min-h-screen">
      <header className="bg-[#0e0e12] fixed top-0 w-full z-50 shadow-[0_40px_60px_-15px_rgba(0,0,0,0.3)]">
        <div className="flex justify-between items-center px-6 py-4 w-full">
          <div className="text-2xl font-black italic tracking-tighter text-[#FF6A00]">
            ATHLO
          </div>
            ...
            <form onSubmit={handlePay} className="space-y-10">
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-full bg-[#ff7a31] flex items-center justify-center text-black font-bold text-sm">
                    1
                  </span>
          ...
          <div className="lg:col-span-5 sticky top-32">
            <div className="bg-[#131317] rounded-3xl p-8 border border-white/5 shadow-2xl relative overflow-hidden">
              ...
              <div className="mt-8 pt-6 border-t border-white/10 space-y-2">
                ...
                <div className="text-gray-400 text-sm">{preview?.coach?.full_name || '-'}</div>
                <div className="text-gray-400 text-sm">
                  {(preview?.coach?.specialites || []).join(' • ') || 'Spécialités non renseignées'}
                </div>
              </div>
            </div>
          </div>
